...
1. *Effects*: [Value initialises](https://en.cppreference.com/w/cpp/language/value_initialization) all members.
...
4. *Preconditions*: Type `InputIt` models *[Cpp17 Input Iterator](https://en.cppreference.com/w/cpp/named_req/InputIterator)* and is indirectly readable as type `N`.
...
10. *Throws*: `std::runtime_error("Cannot call gdwg::graph<N, E>::edges if src or dst node don't exist in the graph")` if either of `is_node(src)` or `is_node(dst)` are `false`.
...
1. *Effects*: Behaves as a [formatted output function](https://en.cppreference.com/w/cpp/named_req/FormattedOutputFunction)</a> of `os`.
...
2. *Remarks*: Pursuant to the requirements of [`std::forward_iterator`](https://en.cppreference.com/w/cpp/iterator/forward_iterator), two value-initialised iterators shall compare equal.
